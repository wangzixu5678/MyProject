...
class TickerColumn {
     ...
     */
    void setTargetChar(char targetChar,boolean singleDigit) {
        ...
        setCharacterIndices(singleDigit);
     ...
     */
    private void setCharacterIndices(boolean singleDigit) {
        currentCharacterList = null;

        for (int i = 0; i < characterLists.length; i++) {
            final TickerCharacterList.CharacterIndices indices =
                    characterLists[i].getCharacterIndices(currentChar, targetChar, metrics.getPreferredScrollingDirection(),singleDigit);
            if (indices != null) {
