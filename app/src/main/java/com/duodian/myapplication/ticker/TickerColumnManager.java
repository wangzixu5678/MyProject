        ...
        for (int i = 0; i < actions.length; i++) {
            switch (actions[i]) {
                ...
                case LevenshteinUtils.ACTION_SAME:
                    tickerColumns.get(columnIndex).setTargetChar(text[textIndex],columnIndex==tickerColumns.size()-1);
                    columnIndex++;
                    textIndex++;
                    break;
                case LevenshteinUtils.ACTION_DELETE:
                    tickerColumns.get(columnIndex).setTargetChar(TickerUtils.EMPTY_CHAR,false);
                    columnIndex++;
                    break;
                default:
