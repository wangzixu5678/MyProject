...
package com.duodian.myapplication.ticker;
...
import com.duodian.myapplication.R;
...
public class TickerView extends View {
    ...
    /**
     * 定制
     * @param startNum  初始的数字
     * @param targetNum 目标数字
     * @param symbol 符号
     */
    public void setText(int startNum,int targetNum,String symbol) {
        int startNumberOfDigits = getNumberOfDigits(startNum);
        int targetNumberOfDigits = getNumberOfDigits(targetNum);
        ...
        if (targetNumberOfDigits != startNumberOfDigits) {
            switch (targetNumberOfDigits) {
                case 3:
                    if (startNumberOfDigits == 1) {
                        // 一位数变三位数
                        setText("00" + startNum + symbol, false);
                    } else {
                        // 两位数变三位数
                        setText("0" + startNum + symbol, false);
                    }
                    break;

                case 2:
                    if (startNumberOfDigits == 3) {
                        // 三位数变两位数
                        setText((startNum / 10) + symbol, false);
                    } else {
                        // 一位数变两位数
                        setText(startNum + "0" + symbol, false);
                    }
                    break;
                ...
                case 1:
                    if (startNumberOfDigits == 3) {
                        // 三位数变一位数
                        setText((startNum / 100) + symbol, false);
                    } else {
                        // 两位数变一位数
                        setText((startNum / 10) + symbol, false);
                    }
        ...
        setText(String.format(Locale.getDefault(),"%d%" + symbol, targetNum));
    }
